...
//mutex semaphore-makes sure that the buffer is only being modified by one thre>
...
static int producer_func(void *data){
        struct pcdata *dat = (struct pcdata *)data;
        int num;
                ...
                struct task_struct *p;
                 for_each_process(p){
                        if(p->cred->uid.val==uid){
                                ...
                                items_produced++;
                                buffer[in] = p;
                                printk(KERN_INFO"[kProducer-1] Produce-Item#:%4>
                                in = (in+1)%buff_size;
                                up(&mutex);
                                up(&full);
                        }
...
int consumer_func(void *data){
        int num;
        struct pcdata *c_dat = (struct pcdata *)data;
        while(!kthread_should_stop()){
                ...
                printk(KERN_INFO"[kConsumer-1] Consumed Item#:%5d on buffer ind>
...
int producer_consumer_init(void){
        ...
        printk(KERN_INFO"module recieved the following inputs: UID:%d,Buffer-Si>
        sema_init(&mutex,1);
        ...
        for(j=0;j<c;j++){
        consumer_thread[j] = kthread_run(consumer_func, NULL, "Consumer Thread >
...
void  producer_consumer_exit(void){
        ...
        time = time % 60000000000;
        seconds = time / 1000000000;
        printk(KERN_INFO"The Total elapsed time of all processes for UID %d is >
